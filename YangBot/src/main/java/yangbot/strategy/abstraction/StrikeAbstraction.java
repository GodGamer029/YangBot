...
public class StrikeAbstraction extends Abstraction {
    ...
    public StrikeAbstraction(Grader grader) {
    ...
    private void solveGoodStrike() {
        ...
        assert this.expectedBallHitTime >= car.elapsedSeconds;
        ...
        Matrix3x3 preorientMatrix;
        {
            ...
            Vector3 comb = orientDir.flatten().add(velDir.mul(0.2f)).normalized().withZ(orientDir.z).normalized();
            ...
            for (float delay = duration + 0.05f; delay <= (duration < 0.2f ? 0.3f : this.maxJumpDelay); delay += jumpDelayStep) {
                ...
                for (float angleDiff = (float) (Math.PI * -0.9f); angleDiff < (float) (Math.PI * 0.9f); angleDiff += angleDiffStep) {
                    ...
                    this.testParameters(
                            ...
                            T,
                            (c, b) -> this.evaluateCollisionState(c, b, statistics));
                }
            }
        }
